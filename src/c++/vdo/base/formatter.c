...
static void vdo_log_capacity(struct vdo *vdo)
{
	...
	slab_count_t slab_count = vdo_compute_slab_count(vdo->states.slab_depot.first_block,
	...
	get_readable_size(slab_config.slab_blocks * VDO_BLOCK_SIZE, &whole, &remaining, &unit);
	vdo_log_debug("The VDO volume can address %llu.%llu %s in %u data slab(s), each %llu.%llu %s",
		      t_whole, t_remaining, UNITS[t_unit], slab_count, whole, remaining, UNITS[unit]);
...
/**
 * write_block() - Synhronously wite a block of data to a vdo's underlying
 *                 block device.
				    ...
				    enum vio_priority prio)
{
	 ...
	 */
	result = vio_reset_bio(vio, block, NULL, REQ_OP_WRITE | REQ_PREFLUSH | REQ_FUA, pbn);
	if (result != VDO_SUCCESS) {
					...
					enum partition_id id)
{
	...
	for (block_count_t n = partition->count; (buffer_blocks < 4096) && ((n & 0x1) == 0); n >>= 1) {
		buffer_blocks <<= 1;
	}
...
{
	int result = clear_volume_geometry(vdo);
	if (result != VDO_SUCCESS) {
...
static int initialize_layout(struct vdo *vdo, nonce_t nonce, char **error_ptr)
{
	...
	physical_block_number_t starting_offset = vdo_get_data_region_start(vdo->geometry) + 1;
...
static int initialize_vdo(struct vdo *vdo, char **error_ptr)
{
	...
	int result = initialize_volume_geometry(vdo, nonce, &uuid);
	if (result != VDO_SUCCESS) {
		return result;
	}
...
/**
 * format_vdo() - Format a block device to function as a new VDO. This function
