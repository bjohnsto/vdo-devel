...
sub removeFromDevicesFile {
  ...
  $machine->sendCommand("sudo lvmdevices");
  $log->info("LVM devices file output after removing device: " . $machine->getStdout());
...
sub destroy {
  ...
  # Device cleanup in the LVM devices file is now handled centrally in VDOTest.pm
  # after all devices are destroyed to ensure better cleanup reliability.
